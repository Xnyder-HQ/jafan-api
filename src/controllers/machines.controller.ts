...
import {
	...
	dynamicWhere, format_phone_number, zero, generate_machine_reference
} from '../config/config';
...
export default class MachineController {
	async getMachines(req: IGetAuthTypesRequest, res: Response) {
		...
		const total_records = await MACHINE.count();
		...
		try {
			const response = await MACHINE.findAndCountAll({
				attributes: { exclude: ['id'] },
				include: [
					{
						model: USER,
			...
			addLog("Machines", user_unique_id, "Queried Machines", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machines Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machines loaded" }, { ...response, pages: pagination.pages });
			}
		} catch (err: any) {
	...
	async getMachine(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Machines", user_unique_id, "Queried Machine", queryParams);
			if (!response) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machine Not found" }, null);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machine loaded" }, response);
			}
		} catch (err: any) {
			return ServerError(res, { unique_id: user_unique_id, text: err.message }, null);
		}
	...
	async searchMachines(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Machines", user_unique_id, "Searched Machines", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machines Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machines loaded" }, { ...response, pages: pagination.pages });
	...
	async getMachinesSpecifically(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		const total_records = await MACHINE.count({ where: dynamicWhere(queryParams) });
		const pagination = paginate(queryParams.page || parseInt(req.body.page), queryParams.size || parseInt(req.body.size), total_records);
		const orderBy = queryParams.orderBy || req.body.orderBy || "createdAt";
		const sortBy = return_all_letters_uppercase(queryParams.sortBy) || return_all_letters_uppercase(req.body.sortBy) || "DESC";
		...
		try {
			...
			addLog("Machines", user_unique_id, "Queried Machines specifically", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machines Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machines loaded" }, { ...response, pages: pagination.pages });
	...
	async filterMachinesSpecifically(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Machines", user_unique_id, "Filtered Machines specifically", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machines Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Machines loaded" }, { ...response, pages: pagination.pages });
	...
	async publicGetMachines(req: IGetAuthTypesRequest, res: Response) {
		const queryParams: IPagination = req.query;

		const total_records = await MACHINE.count({ where: { status: default_status } });
		const pagination = paginate(queryParams.page || parseInt(req.body.page), queryParams.size || parseInt(req.body.size), total_records);
		const orderBy = queryParams.orderBy || req.body.orderBy || "createdAt";
		const sortBy = return_all_letters_uppercase(queryParams.sortBy) || return_all_letters_uppercase(req.body.sortBy) || "DESC";
		...
		try {
			...
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: anonymous, text: "Machines Not found" }, []);
			} else {
				// return SuccessResponse(res, { unique_id: anonymous, text: "Machines loaded" }, { ...response, pages: pagination.pages });
				return SuccessResponse(res, { unique_id: anonymous, text: "Machines loaded" }, { ...response });
			}
		} catch (err: any) {
	...
	async publicGetMachine(req: IGetAuthTypesRequest, res: Response) {
		const errors = validationResult(req);
		const payload = matchedData(req);
		...
		try {
			...
			if (!response) {
				return SuccessResponse(res, { unique_id: anonymous, text: "Machine Not found" }, null);
			} else {
				return SuccessResponse(res, { unique_id: anonymous, text: "Machine loaded" }, response);
			}
		} catch (err: any) {
			return ServerError(res, { unique_id: anonymous, text: err.message }, null);
		}
	...
	async publicSearchMachines(req: IGetAuthTypesRequest, res: Response) {
		const queryParams: IPagination = req.query;
	...
	async addMachine(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			const machine_unique_id = uuidv4();

			await MACHINE.sequelize?.transaction(async (transaction) => {
				const machineResponse = await MACHINE.create({
					unique_id: machine_unique_id,
					reference: generate_machine_reference(), 
					name: payload.name,
					code: payload.code,
					type: payload.type,
					description: payload.description ? payload.description : null,
					supported_block_types: payload.supported_block_types,
					expected_blocks_per_day: payload.expected_blocks_per_day ? parseInt(payload.expected_blocks_per_day) : null,
					fuel_type: payload.fuel_type,
					installed_date: payload.installed_date ? payload.installed_date : null,
					is_active: payload.is_active ? payload.is_active : false_status,
					created_by: user_unique_id,
					status: default_status
				}, { transaction });

				addLog("Machines", user_unique_id, "Added Machine", payload);
				if (machineResponse) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Machine created successfully!" }, { unique_id: machine_unique_id });
				} else {
	...
	async updateMachineDetails(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await MACHINE.sequelize?.transaction(async (transaction) => {
				const response = await MACHINE.update(
					{
						name: payload.name,
						code: payload.code,
						type: payload.type,
				...
				addLog("Machines", user_unique_id, "Updated Machine Details", payload);
				if (response[0] > 0) {
	...
	async updateMachineOtherDetails(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await MACHINE.sequelize?.transaction(async (transaction) => {
				...
				addLog("Machines", user_unique_id, "Updated Machine Other Details", payload);
				if (response[0] > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Details updated successfully!" }, null);
				} else {
					throw new Error("Machine not found");
	...
	async updateMachineDescription(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await MACHINE.sequelize?.transaction(async (transaction) => {
				...
				addLog("Machines", user_unique_id, "Updated Machine Description", payload);
				if (response[0] > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Details updated successfully!" }, null);
				} else {
					throw new Error("Machine not found");
	...
	async updateMachineSupportedBlockTypes(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await MACHINE.sequelize?.transaction(async (transaction) => {
				...
				addLog("Machines", user_unique_id, "Updated Machine Supported Block Types", payload);
				if (response[0] > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Details updated successfully!" }, null);
				} else {
					throw new Error("Machine not found");
	...
	async updateMachineToggles(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await MACHINE.sequelize?.transaction(async (transaction) => {
				...
				addLog("Machines", user_unique_id, "Updated Machine Toggles", payload);
				if (response[0] > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Details updated successfully!" }, null);
				} else {
					throw new Error("Machine not found");
	...
	async deleteMachine(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await MACHINE.sequelize?.transaction(async (transaction) => {
				const response = await MACHINE.destroy(
					{
						where: {
							unique_id: payload.unique_id,
							status: default_status
				...
				addLog("Machines", user_unique_id, "Deleted Machine", payload);
				if (response > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Machine was deleted successfully!" }, null);
				} else {
