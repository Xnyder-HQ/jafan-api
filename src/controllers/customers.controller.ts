...
import {
	...
	dynamicWhere, format_phone_number, zero, return_all_letters_lowercase, generate_customer_reference
} from '../config/config';
...
export default class CustomerController {
	async getCustomers(req: IGetAuthTypesRequest, res: Response) {
		...
		const total_records = await CUSTOMER.count();
		...
		try {
			...
			addLog("Customers", user_unique_id, "Queried Customers", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customers Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customers loaded" }, { ...response, pages: pagination.pages });
			}
		} catch (err: any) {
	...
	async getCustomer(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Customers", user_unique_id, "Queried Customer", queryParams);
			if (!response) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customer Not found" }, null);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customer loaded" }, response);
			}
		} catch (err: any) {
			return ServerError(res, { unique_id: user_unique_id, text: err.message }, null);
		}
	...
	async searchCustomers(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Customers", user_unique_id, "Searched Customers", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customers Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customers loaded" }, { ...response, pages: pagination.pages });
	...
	async getCustomersSpecifically(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		const total_records = await CUSTOMER.count({ where: dynamicWhere(queryParams) });
		const pagination = paginate(queryParams.page || parseInt(req.body.page), queryParams.size || parseInt(req.body.size), total_records);
		const orderBy = queryParams.orderBy || req.body.orderBy || "createdAt";
		const sortBy = return_all_letters_uppercase(queryParams.sortBy) || return_all_letters_uppercase(req.body.sortBy) || "DESC";
		...
		try {
			...
			addLog("Customers", user_unique_id, "Queried Customers specifically", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customers Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customers loaded" }, { ...response, pages: pagination.pages });
	...
	async filterCustomersSpecifically(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Customers", user_unique_id, "Filtered Customers specifically", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customers Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Customers loaded" }, { ...response, pages: pagination.pages });
	...
	async publicGetCustomers(req: IGetAuthTypesRequest, res: Response) {
		const queryParams: IPagination = req.query;

		const total_records = await CUSTOMER.count({ where: { status: default_status } });
		const pagination = paginate(queryParams.page || parseInt(req.body.page), queryParams.size || parseInt(req.body.size), total_records);
		const orderBy = queryParams.orderBy || req.body.orderBy || "createdAt";
		const sortBy = return_all_letters_uppercase(queryParams.sortBy) || return_all_letters_uppercase(req.body.sortBy) || "DESC";
		...
		try {
			...
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: anonymous, text: "Customers Not found" }, []);
			} else {
				// return SuccessResponse(res, { unique_id: anonymous, text: "Customers loaded" }, { ...response, pages: pagination.pages });
				return SuccessResponse(res, { unique_id: anonymous, text: "Customers loaded" }, { ...response });
			}
		} catch (err: any) {
	...
	async publicGetCustomer(req: IGetAuthTypesRequest, res: Response) {
		const errors = validationResult(req);
		const payload = matchedData(req);
		...
		try {
			...
			if (!response) {
				return SuccessResponse(res, { unique_id: anonymous, text: "Customer Not found" }, null);
			} else {
				return SuccessResponse(res, { unique_id: anonymous, text: "Customer loaded" }, response);
			}
		} catch (err: any) {
			return ServerError(res, { unique_id: anonymous, text: err.message }, null);
		}
	...
	async publicSearchCustomers(req: IGetAuthTypesRequest, res: Response) {
		const queryParams: IPagination = req.query;
	...
	async addCustomer(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			const customer_unique_id = uuidv4();

			await CUSTOMER.sequelize?.transaction(async (transaction) => {
				const customerResponse = await CUSTOMER.create({
					...
					phone_number: payload.phone_number ? format_phone_number(payload.phone_number) : null,
					alt_phone_number: payload.alt_phone_number ? format_phone_number(payload.alt_phone_number) : null,
					billing_address: payload.billing_address ? payload.billing_address : null,
					other_address: payload.other_address ? payload.other_address : null,
					balance: payload.balance ? parseInt(payload.balance) : zero,
				...
				addLog("Customers", user_unique_id, "Added Customer", payload);
				if (customerResponse) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Customer created successfully!" }, { unique_id: customer_unique_id });
				} else {
	...
	async updateCustomerDetails(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await CUSTOMER.sequelize?.transaction(async (transaction) => {
				...
				addLog("Customers", user_unique_id, "Updated Customer Details", payload);
				if (response[0] > 0) {
	...
	async updateCustomerAddress(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await CUSTOMER.sequelize?.transaction(async (transaction) => {
				...
				addLog("Customers", user_unique_id, "Updated Customer Address", payload);
				if (response[0] > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Details updated successfully!" }, null);
				} else {
					throw new Error("Customer not found");
	...
	async updateCustomerProfileImage(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			await CUSTOMER.sequelize?.transaction(async (transaction) => {
				...
				addLog("Customers", user_unique_id, "Updated Customer Profile Image", payload);
				if (response[0] > 0) {
					SuccessResponse(res, { unique_id: user_unique_id, text: "Details updated successfully!" }, null);
					
					// Delete former image available
					if (customer_details.profile_image_public_id !== null) {
						await deleteImage(clouder_key || '', { cloudinary_name: cloudy_name, cloudinary_key: cloudy_key, cloudinary_secret: cloudy_secret, public_id: customer_details.profile_image_public_id });
					}
				} else {
					throw new Error("Customer not found");
				}
	...
	async updateCustomerBalance(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await CUSTOMER.sequelize?.transaction(async (transaction) => {
				const response = await CUSTOMER.update(
					{
						balance: parseInt(payload.balance),
					}, {
						where: {
							unique_id: payload.unique_id,
							status: default_status
				...
				addLog("Customers", user_unique_id, "Updated Customer Balance", payload);
				if (response[0] > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Details updated successfully!" }, null);
				} else {
					throw new Error("Customer not found");
	...
	async deleteCustomer(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await CUSTOMER.sequelize?.transaction(async (transaction) => {
				...
				addLog("Customers", user_unique_id, "Deleted Customer", payload);
				if (response > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Customer was deleted successfully!" }, null);
				} else {
