...
export async function addLog(type: string, user_unique_id?: string, description?: string, content?: any, transaction?: any | null | undefined) {
	return transaction ? await LOG.create({
		unique_id: uuidv4(),
...
export default class LogController {
	async getLogs(req: IGetAuthTypesRequest, res: Response) {
		...
		const total_records = await LOG.count();
		...
		try {
			...
			addLog("Logs", user_unique_id, "Queried Logs", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Logs Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Logs loaded" }, { ...response, pages: pagination.pages });
			}
		} catch (err: any) {
	...
	async getLog(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Logs", user_unique_id, "Queried Log", queryParams);
			if (!response) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Log Not found" }, null);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Log loaded" }, response);
			}
		} catch (err: any) {
			return ServerError(res, { unique_id: user_unique_id, text: err.message }, null);
		}
	...
	async searchLogs(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Logs", user_unique_id, "Searched Logs", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Logs Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Logs loaded" }, { ...response, pages: pagination.pages });
	...
	async getLogsSpecifically(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		const total_records = await LOG.count({ where: dynamicWhere(queryParams) });
		const pagination = paginate(queryParams.page || parseInt(req.body.page), queryParams.size || parseInt(req.body.size), total_records);
		const orderBy = queryParams.orderBy || req.body.orderBy || "createdAt";
		const sortBy = return_all_letters_uppercase(queryParams.sortBy) || return_all_letters_uppercase(req.body.sortBy) || "DESC";
		...
		try {
			...
			addLog("Logs", user_unique_id, "Queried Logs Specifically", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Logs Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Logs loaded" }, { ...response, pages: pagination.pages });
	...
	async filterLogsSpecifically(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			...
			addLog("Logs", user_unique_id, "Filtered Logs Specifically", queryParams);
			if (response.count === 0) {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Logs Not found" }, []);
			} else {
				return SuccessResponse(res, { unique_id: user_unique_id, text: "Logs loaded" }, { ...response, pages: pagination.pages });
	...
	async deleteLog(req: IGetAuthTypesRequest, res: Response) {
		const user_unique_id: string = req.USER_UNIQUE_ID;
		...
		try {
			await LOG.sequelize?.transaction(async (transaction) => {
				...
				addLog("Logs", user_unique_id, "Deleted Log", payload, transaction);
				if (response > 0) {
					return SuccessResponse(res, { unique_id: user_unique_id, text: "Log was deleted successfully!" }, null);
				} else {
